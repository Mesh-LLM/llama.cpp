...
static bool ggml_metal_topk_moe_route_pack_enabled() {
    const char * value = getenv("GGML_METAL_ENABLE_TOPK_MOE_ROUTE_PACK");
    if (value && atoi(value) != 0) {
        return true;
    }
    value = getenv("GGML_GLM_DSA_ENABLE_METAL_TOPK_MOE_ROUTE_PACK");
    return value && atoi(value) != 0;
}

static bool ggml_metal_topk_moe_route_pack_unsafe_enabled() {
    const char * value = getenv("GGML_METAL_ENABLE_UNSAFE_TOPK_MOE_ROUTE_PACK");
    if (value && atoi(value) != 0) {
        return true;
    }
    value = getenv("GGML_GLM_DSA_ENABLE_UNSAFE_METAL_TOPK_MOE_ROUTE_PACK");
    return value && atoi(value) != 0;
}
...
static void ggml_metal_log_topk_moe_route_candidate(const ggml_cgraph * gf, int i, const char * reason) {
    ...
    GGML_LOG_INFO(
            "ggml_metal: moe_dispatch op=topk_moe_route_pack tensor=%s candidate=%s reason=%s grid_x=1 grid_y=1 grid_z=1 threads_x=1\n",
            node->name,
            ops,
            reason);
}
