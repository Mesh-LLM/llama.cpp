...
class GGUFWriter:
    ...
    def _get_temp_file(self, shard_idx: int) -> IO[bytes]:
        while len(self.temp_files) <= shard_idx:
            self.temp_files.append(None)
        while len(self.temp_file_advise_offsets) <= shard_idx:
            self.temp_file_advise_offsets.append(0)
        ...
        if fp is None:
            ...
            if _env_bool("GGUF_WRITER_NAMED_TEMP_FILES"):
                fp = tempfile.NamedTemporaryFile(
                    mode="w+b",
                    dir=temp_dir,
                    prefix=f"gguf-shard-{shard_idx + 1:05d}-",
                    suffix=".tmp",
                    delete=True,
                )
            else:
            ...
            _memory_profile(
                "writer_temp_file_open",
                shard_idx=shard_idx,
                temp_dir=temp_dir or tempfile.gettempdir(),
                path=getattr(fp, "name", "unknown"),
            )
    ...
    def add_tensor(
        ...
        self._drop_temp_file_cache(temp_file, shard_idx)
